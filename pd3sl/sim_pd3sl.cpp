...
Sim_pd3sl::Sim_pd3sl(size_t width) : Simulation(width), data2(width * width)
{	/* upon the creation of an object of this class, the following happens: */
	...
	auto changedEvent = [=] (double) {		/* UC: the payoff matrix is defined here, it must be defined in this fashion to work */
        pm[0][0] = 0; pm[0][1] = T; pm[0][2] = sigma;
        pm[1][0] = S; pm[1][1] = 1; pm[1][2] = sigma;
        pm[2][0] = sigma; pm[2][1] = sigma; pm[2][2] = sigma;
    };

    T.onChanged(changedEvent);
    S.onChanged(changedEvent);
    sigma.onChanged(changedEvent);
    changedEvent(0);
...
std::vector<SimParameter> Sim_pd3sl::parameters()
{
	return {startselect, Temp, T, S, sigma, xx, yy};		/* UC: adjust this list to only show the parameters you actually have in your simulation. */
}

uint32_t Sim_pd3sl::renderFrameSkip() const
{
    return 1;		/* UC: returns the frame skip of the simulation(how many steps are done in the background) */
}

size_t Sim_pd3sl::displayWidth() const
{
	return 120;		/* UC: returns the default display width of the simulation. */
}

uint8_t Sim_pd3sl::connectionType() const
{
	return 0;		/* UC: returns the default connection system of the simulation. 0: square lattice 1: triangle lattice 2: random graph*/
}

size_t Sim_pd3sl::resetType() const
{
...
std::string Sim_pd3sl::getName() const
{
	return "PD3sL";		/* UC: returns the name of the simulation for internal purposes. */
}

uint32_t Sim_pd3sl::animationDelay() const
{
    return 200;		/* UC: returns the animation delay of the simulation(how long it waits before displaying the next state, in ms) */
}

void Sim_pd3sl::step()		/* defines a simulation step */
{
	 //altstep();
	 ...
	 for(size_t ii = 0; ii < width() * width(); ii++)
	  {
	   ...
	   auto r3 = rng.nextNormal();
	   if (r3 < xx)
		   partner = player(rng.next() % width(), rng.next() % width());
	   ...
	   if (SXD != SY) {
		  ...
		  if ( r < prd) {
			   at(ixd, jxd) = SY;
		  auto r1 = rng.nextNormal();
		  if (r1 < yy)
			  at(ixd, jxd) = rng.next() % Ns;
		  }
	  }
}
}

void Sim_pd3sl::reset()		/* creates a specific strategy distribution across the grid */
{
...
size_t Sim_pd3sl::returnSpecies()
{
	return Ns;
}  /*MCS*/

size_t Sim_pd3sl::stepTargetNumber() const
{
