...
Sim_socdil_mix_logit::Sim_socdil_mix_logit(size_t width) : Simulation(width), data2(width * width)
...
{
	return {startselect, T, R, S, P, w, zz, Temp};		/* UC: adjust this list to only show the parameters you actually have in your simulation. */
}
...
std::string Sim_socdil_mix_logit::getName() const
{
	return "socdil_mix_logit";			/* UC: returns the name of the simulation for internal purposes. */
}
...
void Sim_socdil_mix_logit::reset()		/* creates a specific strategy distribution across the grid */
{
...
size_t Sim_socdil_mix_logit::returnSpecies()
{
	return Ns;
}
